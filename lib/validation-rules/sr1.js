...
const isValidPatientName = (value, { fieldName }) => {
  ...
  if (isValid) {
    ...
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.endCharInvalid`
      }
    })]
  } else if (!isValidSpecialChar) {
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.specialChar`
      }
    })];
  } else {
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.pattern`
      }
    })];
  }
}
...
function checkWordsValid (value, lengthLimit, { fieldName }) {
  ...
  if (isValid) {
    ...
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.wordTooLong`,
        inline: `sr1:${fieldName}.wordTooLong`
      }
    })]
  }
}
...
function hasValidWordsPatientName (value, { fieldName }) {
  ...
  if (isValid) {
    ...
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.firstLastTooLong`,
        inline: `sr1:${fieldName}.firstLastTooLong`
      }
    })];
  } else if (isFirstWordTooShort) {
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.wordTooShort`,
        inline: `sr1:${fieldName}.wordTooShort`
      }
    })];
  } else if (!isValidFirstWords) {
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.firstWordTooLong`,
        inline: `sr1:${fieldName}.firstWordTooLong`
      }
    })];
  } else if (!isValidLastWord) {
    return [ValidationError.make({
      errorMsg: {
        summary: `sr1:${fieldName}.lastWordTooLong`,
        inline: `sr1:${fieldName}.lastWordTooLong`
      }
    })];
  }
}
